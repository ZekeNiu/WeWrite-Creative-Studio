...
export function Field({label,value,onCommit,placeholder='',multiline=false,type='text',min,max,hint,autoSave=false}:{label:string;value:string|number;onCommit:(s:string)=>void|Promise<unknown>;placeholder?:string;multiline?:boolean;type?:string;min?:number;max?:number;hint?:string;autoSave?:boolean}){
 ...
 async function commit(){
  ...
  const task=(async()=>{try{await commitRef.current(text);if(draft.current===text)dirty.current=false;notifyFields();setError('')}catch(e){setError('未保存，请重试；输入已保留');throw e}})();
...
export function Toggle({checked,onChange,label,small=false}:{checked:boolean;onChange:(v:boolean)=>void|Promise<unknown>;label:string;small?:boolean}){
 const [local,setLocal]=useState(checked),[error,setError]=useState('');const draft=useRef(checked),dirty=useRef(false),pending=useRef<Promise<void>|null>(null),handler=useRef(onChange);handler.current=onChange;
 useEffect(()=>{if(!dirty.current){draft.current=checked;setLocal(checked)}},[checked]);
 async function commit(){if(pending.current)await pending.current;if(!dirty.current)return;const value=draft.current;const task=(async()=>{try{await handler.current(value);if(draft.current===value)dirty.current=false;notifyFields();setError('')}catch(e){setError('未保存，选择已保留');throw e}finally{notifyFields()}})();pending.current=task;try{await task}finally{pending.current=null}}
 const flush=useRef(commit);flush.current=commit;useEffect(()=>registerField(()=>flush.current(),()=>dirty.current),[]);
 return <label className={'toggle-label '+(small?'small':'')}><button type="button" className={'toggle '+(local?'on':'')} role="switch" aria-checked={local} aria-label={label} onClick={()=>{draft.current=!draft.current;dirty.current=true;setLocal(draft.current);notifyFields();void commit().catch(()=>{})}}><span/></button><span>{label}</span>{error&&<small role="alert">{error}<button type="button" onClick={()=>void commit().catch(()=>{})}>重试保存</button></small>}</label>
}
export function Select({label,value,onChange,children}:{label:string;value:string;onChange:(s:string)=>void|Promise<unknown>;children:ReactNode}){
 ...
 async function commit(){
  ...
  const task=(async()=>{try{await handler.current(next);if(draft.current===next)dirty.current=false;notifyFields();setError('')}catch(e){setError('未保存，选择已保留');throw e}})();
  pending.current=task;try{await task}finally{if(pending.current===task)pending.current=null}
 }
 const flush=useRef(commit);flush.current=commit;
 useEffect(()=>registerField(()=>flush.current(),()=>dirty.current),[]);
 return <label className="field"><span>{label}</span><div className="select-wrap"><select aria-label={label} aria-invalid={!!error} value={local} onChange={e=>{draft.current=e.target.value;dirty.current=true;setLocal(e.target.value);notifyFields();void commit().catch(()=>{})}}>{children}</select><ChevronDown size={14}/></div>{error&&<small role="alert">{error}<button type="button" className="text-button" onClick={()=>void commit().catch(()=>{})}>重试保存</button></small>}</label>
}
export function Modal({title,children,onClose,wide=false}:{title:string;children:ReactNode;onClose:()=>void;wide?:boolean}){
 const heading=useId();const ref=useRef<HTMLDialogElement>(null);useEffect(()=>{const previous=document.activeElement as HTMLElement|null;ref.current?.showModal();return()=>{if(previous?.isConnected)previous.focus()}},[]);
