...
export default function StageContext({a,step,selection,clear,inspect,locate,issue}:Props){
 ...
 if(step==='sources'){
  ...
  const issues=a.materials_state?a.materials_state.pending||[]:(a.research?.issues||[]).filter(i=>['open','stale'].includes(i.status));
  return <div className="stage-context">{s?<><button className="text-button" onClick={clear}>返回素材概况</button><h3>{s.title}</h3><Tag tone={s.selected?'':'amber'}>{s.selected?'已采用':'未采用'}</Tag>{history}<button className="button secondary full" onClick={()=>inspect(s.id)}>引用详情与原文依据</button><h4>关联主张</h4>{claimList(related.map(c=>c.id))}<h4>关联章节</h4>{relatedSections.length?relatedSections.map(section=><p key={section.id}>{section.title}</p>):<p className="muted">尚无关联章节。</p>}</>:<><h3>素材参考</h3>{history}<p>选择主区的一条素材，查看关联主张、章节和原文。</p><p className="muted">当前结论与待处理建议集中在主区“整理结果”。</p>{issues.length>0&&<button className="button secondary" onClick={()=>issue(issues[0].id)}>查看待处理建议 · {issues.length}</button>}</>}<p className="context-footnote">关联来自已有资料整理与大纲，不代表正文已实际引用。完整编辑在主区进行。</p></div>;
 }
 if(step==='outline'){
