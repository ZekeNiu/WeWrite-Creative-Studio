...
export default function LayoutPanel({a,save,act,run,busy,meta,prepareExport}:{a:Article;save:Save;act:Action;run:Run;busy:boolean;meta:Meta;prepareExport:(open?:boolean)=>Promise<any>}){
 ...
 return <div className="layout-workspace"><div className="layout-controls">
  ...
  <Select label="排版主题" value={layout.theme} onChange={v=>{const t=meta.themes.find(t=>t.id===v);if(t)choose(t)}}>{meta.themes.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</Select>
  <div className="theme-swatches">{meta.themes.slice(0,12).map(t=><button title={t.name} aria-label={t.name} key={t.id} style={{background:t.colors.primary||'#243e38'}} className={layout.theme===t.id?'selected':''} onClick={()=>choose(t)}/>)}</div>
  ...
  <p className="muted small-text">切换主题会采用推荐字号与间距。<button className="text-button" disabled={!theme||saving>0} onClick={()=>theme&&choose(theme)}>恢复推荐设置</button></p>
