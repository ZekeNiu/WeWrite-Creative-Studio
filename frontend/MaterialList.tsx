...
export default function MaterialList({a,save,busy,act,inspect,onContext,prepare}:{a:Article;save:Save;busy:boolean;act:(fn:()=>Promise<void>)=>Promise<void>;inspect:(id:string)=>void;onContext?:(id:string,explicit?:boolean)=>void;prepare?:()=>Promise<Article>}){
 ...
 <div className="row wrap material-tags"><Tag>{s.status==='metadata_only'?'仅文献信息':s.status==='abstract_only'?'研究摘要':s.status==='excerpt_only'?'搜索片段':s.status==='unreadable'?'无法读取':s.kind==='user'?'用户提供':'已读正文'}</Tag>{manual&&<Tag>有使用要求</Tag>}{s.personal_material&&<Tag>已授权个人经历</Tag>}<small className="muted">{s.id}</small></div>
