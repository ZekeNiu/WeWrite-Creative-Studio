...
import SourceNotebook from './SourceNotebook';
...
export default function MaterialList({a,save,busy,act,inspect,onContext,prepare}:{a:Article;save:Save;busy:boolean;act:(fn:()=>Promise<void>)=>Promise<void>;inspect:(id:string)=>void;onContext?:(id:string,explicit?:boolean)=>void;prepare?:()=>Promise<Article>}){
 ...
 const change=(patch:Partial<typeof view>)=>{void act(async()=>{await prepare?.();setView(old=>{const next={...old,...patch};remember(key,next);return next})})};
 ...
 async function commit(ids:string[],patch:Record<string,unknown>,remove=false,flush=true){
  if(lock.current||(busy&&flush))return;
  if(flush&&prepare){try{await prepare()}catch{return}}
  lock.current=true;setSaving(true);
  try{await save(current=>({sources:current.sources.filter(s=>!remove||!ids.includes(s.id)).map(s=>({id:s.id,...(ids.includes(s.id)?patch:{})})) as Article['sources']}),'sources')}catch(e){if(!flush)throw e}
 ...
 <div className="source-list">{visible.map(s=>{const open=view.open.includes(s.id);const manual=!!s.use?.trim();const claims=(a.evidence.claims||[]).filter((c:any)=>c.source_ids?.includes(s.id));const sections=(a.outline.sections||[]).filter(section=>section.claim_ids?.some(id=>claims.some((c:any)=>c.id===id)));const stale=a.stages.sources==='stale'||!!a.research?.stale;return <article key={s.id} className={'source-card compact-source '+(!s.selected?'excluded':'')}>
 <div className="row"><input type="checkbox" aria-label={'采用 '+s.title} disabled={disabled} checked={s.selected} onChange={e=>commit([s.id],{selected:e.target.checked})}/><FileText size={18}/><button className="source-title grow" aria-expanded={open} aria-controls={'source-'+s.id} onClick={()=>{change({open:open?view.open.filter(id=>id!==s.id):[...view.open,s.id]});if(!open)onContext?.(s.id)}}>{s.title}<ChevronDown size={16} className={open?'rotated':''}/></button><button className="text-button" onClick={()=>onContext?.(s.id,true)}>查看依据</button><button className="text-button" onClick={()=>inspect(s.id)}>引用详情</button>{s.url&&<a href={s.url} target="_blank" rel="noreferrer" className="icon-button" title="查看原网页"><ExternalLink size={16}/></a>}<button className="icon-button" disabled={disabled} title="移除素材" onClick={()=>commit([s.id],{},true)}><Trash2 size={15}/></button></div>
 <div className="row wrap material-tags"><Tag>{s.status==='metadata_only'?'仅文献信息':s.status==='abstract_only'?'研究摘要':s.status==='excerpt_only'?'搜索片段':s.status==='unreadable'?'无法读取':s.kind==='user'?'用户提供':'已读正文'}</Tag>{manual&&<Tag>有使用要求</Tag>}{s.personal_material&&<Tag>已授权个人经历</Tag>}<small className="muted">{[s.bibliography?.venue,s.bibliography?.year].filter(Boolean).join(" · ")|| (s.original_filename?"文献信息待补全":"")}</small></div>
 <p className={open?'':'preview-clamp'}>{s.summary||'暂无摘要'}</p>
 {open&&<SourceNotebook source={s}/>}
 {open&&<div id={'source-'+s.id} className="material-expanded"><fieldset disabled={disabled}><Field label="使用要求（可选）" value={s.use||''} placeholder="例如：只参考表达风格，不引用其中事实" hint="留空即可。仅填写你希望特别遵守的要求，不代表证据已经核实。" onCommit={use=>commit([s.id],{use},false,false)}/>{manual&&<button className="text-button" onClick={()=>commit([s.id],{use:''})}>清空使用要求</button>}<details className="material-relations"><summary>关联主张与大纲章节{stale?'（历史关联，需更新）':''}</summary>{claims.length?claims.map((c:any)=><p key={c.id}>{c.text}{c.status==='unsupported'?'（缺少支持）':''}{c.boundary?' · '+c.boundary:''}</p>):<p className="muted">尚无关联主张，整理素材后可查看。</p>}{sections.length>0&&<><strong>关联大纲章节{a.stages.outline==='stale'?'（待更新）':''}</strong><ul>{sections.map(section=><li key={section.id}>{section.title}</li>)}</ul></>}<small className="muted">以上来自已有证据与大纲，不代表正文已实际引用。</small></details>{s.kind==='user'&&<div className="experience-consent"><strong>个人经历授权</strong><label><input type="checkbox" checked={!!s.personal_material} onChange={e=>commit([s.id],{personal_material:e.target.checked})}/>这是我的真实经历，可用于第一人称叙述</label><small className="muted">上传的论文、范文或他人经历不等于你的亲身经历。AI 不会自动开启此项。</small></div>}</fieldset></div>}</article>})}</div><Pager page={page} total={rows.length} onChange={page=>change({page})}/></>
}
