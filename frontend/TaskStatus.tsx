...
export function StageService({cfg,stage,onSettings}:{cfg:Settings;stage:Stage;onSettings:(stage:string)=>void}){
 ...
 return <>{[stage==='sources'?'research':stage,'search'].map(key=>{
  const {service,model,origin}=effectiveService(cfg,key),label=key==='search'?'联网搜索':key==='research'?'规划与整理':LABELS[key];
  const analysis=key==='research'?effectiveService(cfg,'sources'):null;
  return <div className="stage-service" key={key}><span className="service-name">{label}：{key==='search'&&!cfg.search.enabled?'已关闭':service?`${service.name} · ${model||'未填写模型'}`:'尚未配置模型服务'} </span><details className="service-origin"><summary>配置来源</summary><p>{origin}</p>{analysis&&<p>素材分析：{analysis.service?.name||'未配置'} · {analysis.model}</p>}</details><button className="text-button" onClick={()=>onSettings(key)}>调整{label}服务</button></div>;
 })}</>;
}
