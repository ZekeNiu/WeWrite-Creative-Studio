...
export const ERROR_LABELS:Record<string,string>={timeout:'等待响应超时',connection:'连接中断',quota:'余额或额度不足',rate_limit:'请求限流',rate_limit_or_quota:'限制原因未明确',authentication:'凭证错误',permission:'权限不足',invalid_request:'请求不被接受',not_found:'模型或接口不存在',service_error:'模型服务错误',validation:'输入或结果需要调整',budget_exceeded:'达到任务预算上限',unknown:'未分类错误',empty_response:'接口返回空结果',missing_tool_call:'未返回要求的工具调用',output_truncated:'输出达到上限而截断',malformed_response:'响应格式异常',refusal:'模型明确拒答',search_evidence_missing:'未取得真实搜索证据',search_limit_exceeded:'搜索内部次数超出限制',search_unavailable:'搜索工具暂时不可用',search_tool_error:'搜索工具返回错误',search_incomplete:'搜索尚未完成'};
