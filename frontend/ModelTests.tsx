...
export function modelCards(cfg:Settings):ModelCard[]{
 ...
 ['topic','sources','research','outline','write','review','revise','visual','image','vision','layout_advice'].forEach(role=>{const r=cfg.routes[role]||{};const inherited=!r.service_id?(role==='research'?cfg.routes.sources:role==='vision'?cfg.routes.visual:undefined):undefined;add(r.service_id||inherited?.service_id||cfg.default_service,r.model||inherited?.model||'',role)});
...
export default function ModelTests({cfg,busy,onProtocol,onTest}:{cfg:Settings;busy:boolean;onProtocol:(card:ModelCard,protocol:string)=>void;onTest:(card:ModelCard,kind:string)=>void}){
 ...
 return <><div className="section-intro"><h3>按模型验证能力</h3><p>每次只测试点击的模型与能力，不改变创作分工。测试会真实调用接口，可能产生费用；图片测试生成 1 张图片。</p></div>
 ...
 <div className="model-capabilities">{[['text','测试文本连接'],['image','测试图片生成'],['search','测试联网搜索'],['vision','测试识图']].map(([kind,label])=>{const r=c.capabilities[kind];return <div className="model-capability" key={kind}><button className="button secondary" disabled={busy||!c.model} onClick={()=>onTest(c,kind)}>{label}</button><Tag tone={r?.status==='tested'?'green':r?.status==='failed'?'amber':''}>{STATES[r?.status||'untested']||'尚未测试'}</Tag>{r?.at&&<small className="muted">{new Date(r.at).toLocaleString()}</small>}{r?.message&&<p className="small-text">{r.message}</p>}</div>})}</div>
