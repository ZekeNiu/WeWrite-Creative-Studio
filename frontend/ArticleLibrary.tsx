...
import {LABELS,type Article} from './types';
...
export default function ArticleLibrary({articles,onOpen,onChanged}:{articles:Article[];onOpen:(id:string)=>void;onChanged:()=>Promise<void>}){
 const [trash,setTrash]=useState(false),[trashed,setTrashed]=useState<Article[]>([]),[confirm,setConfirm]=useState<Article|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState('');
 ...
 async function reload(){setTrashed(await api('/articles?state=trash'));await onChanged()}
 const rows=trash?trashed:articles;
 return <section className="library"><div className="row between wrap"><div className="row"><h2>{trash?'回收站':'我的文章'}</h2><span className="count">{rows.length}</span></div><button className="text-button" disabled={busy} onClick={()=>void action(async()=>{if(!trash)setTrashed(await api('/articles?state=trash'));setTrash(!trash);setNotice('')})}>{trash?'返回文章库':'回收站'}</button></div>
