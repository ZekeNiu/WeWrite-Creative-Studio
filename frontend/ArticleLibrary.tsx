...
export default function ArticleLibrary({articles,onOpen,onChanged}:{articles:Article[];onOpen:(id:string)=>void;onChanged:()=>Promise<void>}){
 ...
 {rows.length?<div className="article-grid">{rows.map(a=><article className="article-card library-card" key={a.id}><button className="article-open" disabled={trash||busy} aria-label={'打开文章：'+a.title} onClick={()=>onOpen(a.id)}><div className="row between"><Tag>{a.brief.column}</Tag>{!trash&&<ArrowUpRight size={16}/>}</div><h3>{a.title}</h3><p>{a.brief.audience} · 正文 {a.word_count??0} 字 / 目标 {a.brief.words} 字</p></button><div className="article-card-bottom"><span>{trash?'已移入回收站':LABELS[a.current_stage]||'创作中'}{!trash&&a.latest_job&&<small className={a.latest_job.status==='failed'?'library-failure':''}>{LABELS[a.latest_job.stage]||'任务'} · {JOB_STATUS[a.latest_job.status]||'待查看'}</small>}</span><time>{date(a.updated)}</time></div><div className="library-actions">{trash?<><button className="text-button" disabled={busy} onClick={()=>void action(async()=>{await api('/articles/'+a.id+'/untrash','POST',{revision:a.revision});await reload();setNotice('文章已恢复到文章库')})}><RotateCcw size={14}/>恢复</button><button className="text-button" disabled={busy} onClick={()=>setConfirm(a)}><Trash2 size={14}/>彻底删除</button></>:<button className="text-button" disabled={busy} aria-label={'删除文章：'+a.title} onClick={()=>void action(async()=>{await api('/articles/'+a.id+'/trash','POST',{revision:a.revision});await reload();setNotice('已移入回收站，可随时恢复')})}><Trash2 size={14}/>删除</button>}</div></article>)}</div>:<div className="library-empty"><FolderOpen size={28}/><div><h3>{query.trim()?'没有匹配的文章':trash?'回收站为空':'这里，留给你的下一篇作品'}</h3><p>{query.trim()?'试试其他关键词，或清空搜索查看全部文章。':trash?'删除的文章会先保留在这里。':'新建文章或导入旧稿，随时回来继续。'}</p></div></div>}
