...
export default function Sources({a,save,act,update,run,busy,onJob,job,focusRequest,onFocusHandled,onContext,prepare,navigate}:Common&{job:Job|null;focusRequest?:IssueFocus;onFocusHandled?:()=>void;onContext?:(id:string,explicit?:boolean)=>void}){
 ...
 const primary=()=>{locate();void run('research',{instruction:query,research_limits:limits,chain:false})};
 ...
 return <>
  ...
  <div className="row wrap source-actions"><button className="button secondary" disabled={busy||localBusy} onClick={()=>{setAttachmentIds([]);file.current?.click()}}><Upload size={15}/>上传文件</button><button className="button secondary" disabled={busy||localBusy} onClick={()=>openMode('url')}><LinkIcon size={15}/>添加链接</button><button className="button secondary" disabled={busy||localBusy} onClick={()=>openMode('text')}><Plus size={15}/>粘贴文字</button>{<button className="text-button" disabled={localBusy} onClick={()=>navigate('outline')}>进入大纲</button>}<span className="muted">PDF / Word / Markdown / TXT / BibTeX / RIS · 20 MB 内</span></div>
  ...
  {limitsOpen&&<SourceLimits a={a} initial={limits} busy={busy} onClose={()=>setLimitsOpen(false)} onApply={async(value,go)=>{await prepare();const current=await save({research_limits:value},'preferences');if(go){const next=await api<Job>('/articles/'+a.id+'/jobs','POST',{stage:'research',revision:current.revision,research_limits:value,research_parent_id:current.research?.job_id||'',issue_ids:state?.required.map(i=>i.id)||[],chain:false});onJob(next);locate()}}}/>}
