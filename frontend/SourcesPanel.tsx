...
import SourceLimits,{type Limits} from './SourceLimits';
...
export default function Sources({a,save,act,update,run,busy,onJob,job,focusRequest,onFocusHandled,onContext,prepare,navigate}:Common&{job:Job|null;focusRequest?:IssueFocus;onFocusHandled?:()=>void;onContext?:(id:string,explicit?:boolean)=>void}){
 ...
 const [attachmentIds,setAttachmentIds]=useState<string[]>([]),[mode,setMode]=useState(''),[url,setUrl]=useState(''),[title,setTitle]=useState(''),[text,setText]=useState(''),[query,setQuery]=useState('');
 ...
 const [limits,setLimits]=useState<Limits>(),[limitsOpen,setLimitsOpen]=useState(false);
 ...
 const executing=!!job&&job.article_id===a.id&&['running','queued'].includes(job.status);
 ...
 const primary=()=>{if(state?.action==='outline'&&!query.trim()){void navigate('outline');return}if(state?.action==='supplement'&&!query.trim()){locate(state.pending?.[0]?.id);return}locate();void run(state?.action==='collect'||!state?'sources':'research',{instruction:query,issue_ids:state?.required.map(x=>x.id)||[],research_limits:limits,chain:a.auto.sources})};
 const primaryLabel=query.trim()?'继续查证':state?.action==='outline'?'进入大纲':state?.action==='verify_new'?'核实新增资料':state?.action==='verify'?'更新相关判断':state?.action==='continue'?'继续查证':state?.action==='supplement'?'查看待处理建议':'查找并整理资料';
 const summary=a.research?.summary||a.evidence.summary;
 return <>
  <section className="material-status" aria-label="资料状态" role="status"><div className="row between"><strong>{executing?job!.message:state?.message||'先检查已有材料，再按需查找'}</strong><button className="text-button" onClick={()=>setLimitsOpen(true)}><SlidersHorizontal size={14}/>检索设置</button></div>
   {executing?<div className="row between"><Busy text={importing?'正在导入资料':'正在处理本次任务'}/><button className="text-button" onClick={()=>void perform(async()=>onJob(await api('/jobs/'+job!.id+'/cancel','POST')))}><Square size={12}/>停止</button></div>:state?.delta&&<p className="muted">上次核实：新增 {state.delta.added_sources} 条素材 · 解决 {state.delta.resolved} 项建议 · 剩余 {state.delta.remaining} 项高优先级建议</p>}
  ...
  <div className="source-search"><Search size={18}/><input aria-label="补充检索要求" value={query} onChange={e=>setQuery(e.target.value)} placeholder="可选：补充需要查证的问题…"/><button className="button primary" disabled={busy||localBusy||!a.brief.topic} onClick={primary}>{primaryLabel}</button></div>
  <div className="row wrap source-actions"><button className="button secondary" disabled={busy||localBusy} onClick={()=>{setAttachmentIds([]);file.current?.click()}}><Upload size={15}/>上传文件</button><button className="button secondary" disabled={busy||localBusy} onClick={()=>openMode('url')}><LinkIcon size={15}/>添加链接</button><button className="button secondary" disabled={busy||localBusy} onClick={()=>openMode('text')}><Plus size={15}/>粘贴文字</button>{primaryLabel!=='进入大纲'&&<button className="text-button" disabled={localBusy} onClick={()=>navigate('outline')}>进入大纲</button>}<span className="muted">PDF / Word / Markdown / TXT / BibTeX / RIS · 20 MB 内</span></div>
  ...
  <div role="tabpanel" aria-label="整理结果" hidden={tab!=='results'}>{summary?<section className="research-summary"><div className="row between"><h3>当前结论</h3>{a.research?.stale&&<Tag tone="amber">相关依据待更新</Tag>}</div><p className={summaryOpen?'':'preview-clamp summary-preview'}>{summary}</p><button className="text-button" aria-expanded={summaryOpen} onClick={()=>setSummaryOpen(v=>!v)}>{summaryOpen?'收起整理总结':'展开完整整理总结'}</button></section>:<p className="muted">尚无整理结论，添加材料后可开始核实；也可以先构建大纲。</p>}
   {a.evidence.claims?.length>0&&<details className="evidence-details"><summary>主张与来源依据</summary>{a.evidence.claims.map((c:any)=><div className="claim" key={c.id}><Tag tone={c.status==='unsupported'||c.stale?'amber':''}>{c.stale?'依据待更新':c.status==='unsupported'?'缺少支持':c.status==='bounded'?'适用范围有限':'已有支持记录'}</Tag><p>{c.text}</p><p className="muted">{c.boundary}</p>{c.source_ids.map((id:string)=><button className="text-button source-reference" key={id} onClick={()=>setInspect(id)}>{a.sources.find(s=>s.id===id)?.title||'来源已删除'}</button>)}</div>)}</details>}
  ...
  {limitsOpen&&<SourceLimits a={a} initial={limits} busy={busy} onClose={()=>setLimitsOpen(false)} onApply={async(value,go)=>{const current=await prepare();setLimits(value);if(go){const next=await api<Job>('/articles/'+a.id+'/jobs','POST',{stage:'research',revision:current.revision,research_limits:value,research_parent_id:current.research?.job_id||'',issue_ids:state?.required.map(i=>i.id)||[],chain:false});onJob(next);locate()}}}/>}
  {inspect&&a.sources.find(s=>s.id===inspect)&&<ReferenceDetails key={inspect} source={a.sources.find(s=>s.id===inspect)!} onClose={()=>setInspect(null)} onSave={s=>save(current=>({sources:current.sources.map(x=>x.id===s.id?s:x)}),'sources')} onIdentify={()=>{const id=inspect;setInspect(null);return begin('identify',undefined,id)}} onLookup={async doi=>{const current=await prepare();update(await api(`/articles/${a.id}/sources/${inspect}/metadata`,'POST',{revision:current.revision,doi}))}}/>}
 </>;
}
