...
with sync_playwright() as p:
    ...
    expect(page.locator('.editorial-card')).to_have_count(4)
    expect(page.locator('.reading-advice')).not_to_have_attribute('open','')
    expect(page.get_by_role('button',name='应用专业科普')).to_have_attribute('aria-pressed','true')
    page.screenshot(path=str(OUT/'theme-gallery.png'),full_page=True)
    themes=page.request.get(BASE+'/api/meta').json()['themes'][:4]
    for theme in themes:
        page.get_by_role('button',name='应用'+theme['name'],exact=True).click()
        expect(copy_button).to_be_enabled()
        expect(page.get_by_role('button',name='应用'+theme['name'],exact=True)).to_have_attribute('aria-pressed','true')
    ...
    page.evaluate("""() => {for(const name of ['应用杂志长文','应用人文随笔','应用现代专题'])document.querySelector(`[aria-label="${name}"]`).click()}""")
    ...
    page.locator('.layout-details summary').filter(has_text='排版微调').click()
    ...
    page.get_by_role('button',name='恢复主题推荐设置',exact=True).click();expect(copy_button).to_be_enabled()
    ...
    page.get_by_role('button',name='应用杂志长文').click()
    ...
    page.get_by_role('button',name='应用现代专题').click();expect(copy_button).to_be_enabled()
    page.reload();expect(copy_button).to_be_enabled()
    expect(page.get_by_role('button',name='应用现代专题')).to_have_attribute('aria-pressed','true')
    page.get_by_role('button',name='430像素预览').click()
    assert page.locator('.phone-frame').evaluate('(n)=>n.getBoundingClientRect().width')==430
    ...
    page.get_by_role('button',name='应用人文随笔').click()
    ...
    browser.close()
print('Passed: four themes at 375/430, selection/defaults/refresh, rapid switching, clipboard consistency, optional advice, conflicts, unsaved editor, 1100 layout.')
