...
def model(monkeypatch,client):
    ...
    async def fake(s,system,prompt,emit=None):
        ...
        elif schema=='ReviewResult': result={'decision':'pass','summary':'所给资料内未发现明显矛盾，请人工核对','issues':[],'dimensions':{'准确':4,'深度':4,'自然':4},'digest':'阅读研究，先看证据与边界','title':'理解证据的边界','tags':['研究']}
...
def test_automatic_review_bounded_to_two_passes(client,model,monkeypatch):
    ...
    async def always_bad(s,system,prompt,emit=None):
        counter.append(1);content=json.loads(prompt)['资料与当前内容']['article']
        text=json.dumps({'decision':'revise','summary':'仍然缺乏证据','issues':[{'id':'one','severity':'blocker','quote':content,'reason':'缺少支持','suggestion':'缩小后的判断'}]},ensure_ascii=False)
