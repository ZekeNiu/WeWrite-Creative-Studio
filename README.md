...
安装 Python 3.11 或更新版本，下载本仓库后双击 **启动工作台.bat**。首次启动或依赖锁文件变化时会安装锁定的 Python 依赖并自检；仓库包含预构建界面，日常使用不需要 Node.js。退出后台请使用 **停止工作台.bat**。
...
资料按具体主张评估出处，优先论文、专业书籍、权威机构、官方文档与专业博客；保留原文与可定位证据。运行不设金额预算门槛，保留搜索次数、页数和补查轮数上限。

当前版本 **v1.7.0**，增加按问题研究、来源笔记与有边界的独立核查；见 [交付记录](docs/RELEASE-1.7.0.md)。

完整操作说明见 [使用说明](使用说明.md)，版本记录见 [CHANGELOG](CHANGELOG.md)。

素材页根据当前结论提示下一步：核实新增材料、处理核心缺口或进入大纲。问题可逐项核实、补充原文、采用明确限定表述或本篇不使用；研究局限保留为写作条件。选题反馈和采用的完整方案会持续传入大纲、正文及审核。
...
ZIP 是文章分享包，排除未引用素材全文及内部备注，不能用于完整恢复创作数据。首次写入新压缩历史前，会在 `data/backups/` 创建并校验数据库副本；它不含附件。回退旧程序必须使用对应的旧数据库副本，不能直接打开已含新格式历史的数据库。
...
`dist/` 随源码提交以支持双击运行；修改前端后需重新构建。测试默认使用临时数据目录。`tools/qa_reliability_app.py` 仅用于临时数据的离线模拟界面验收，不由正式启动器加载；另一个终端运行 `tools/check_reliability_ui.py` 可复现完整路径。Windows CI 自动执行后端、类型／构建和 Edge 浏览器检查，不上传数据或凭证。
