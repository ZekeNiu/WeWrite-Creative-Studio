...
INSTRUCTION = (
    ...
    '每个items.source_quote逐字摘录实际已读原文的相应连续条件，不拼接或加省略号；evidence_ids指向同来源、已经独立核查的主张，covered表示该条件及其前提、例外、阈值已被主张保留。引文里存在条件但主张没解释，仍未完成回答。'
        ...
        if not check or not check.get('reason'):
            ...
            for source_list in check['source_lists']:
                ...
                for item in source_list['items']:
                    ...
                    if not ids or set(ids)-valid or not any(evidence.get(eid,{}).get('source_id')==sid and quote in normal(evidence[eid].get('quote',''),pdf) for eid in ids):
                        reasons.append('清单条目尚未关联同来源的有效回答')
